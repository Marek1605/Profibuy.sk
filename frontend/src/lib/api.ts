import type { Product, Category, PaginatedResponse, FilterOptions, ShippingMethod, PaymentMethod, Order, DashboardStats, Feed } from '@/types'
...
export async function getProducts(params?: URLSearchParams): Promise<PaginatedResponse<Product>> {
  ...
  return fetchAPI<PaginatedResponse<Product>>(`/api/products${query}`)
}
