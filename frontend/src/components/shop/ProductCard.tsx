...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="product-card group relative">
      {/* Sale badge */}
      {discount > 0 && (
        <span className="sale-badge">Zlava -{discount}%</span>
      )}
      ...
      <div className="absolute top-3 right-3 flex flex-col gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <button onClick={(e) => { e.preventDefault(); setWishlisted(!wishlisted) }} className={`w-9 h-9 rounded-full bg-white shadow-md flex items-center justify-center transition hover:scale-110 ${wishlisted ? 'text-red-500' : 'text-gray-400 hover:text-red-500'}`}>
          <svg className="w-4.5 h-4.5" fill={wishlisted ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
        </button>
      ...
      <Link href={`/products/${product.slug}`}>
        <div className="aspect-square bg-gray-50 flex items-center justify-center p-6 overflow-hidden">
          <img src={image} alt={product.name} className="max-w-full max-h-full object-contain transition-transform duration-500 group-hover:scale-110" onError={(e) => { (e.target as HTMLImageElement).style.opacity = '0.3' }} loading="lazy" />
        </div>
      </Link>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          <span className={`flex items-center gap-1.5 text-xs font-medium ${product.stock > 0 ? 'text-green-600' : 'text-red-500'}`}>
