      ...
      <div className="border-b">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <Link href="/cart" className="flex items-center gap-2 px-3 py-2 text-gray-700 hover:text-blue-600 relative">
              ...
              {cartCount > 0 && <span className="absolute -top-0.5 left-5 w-5 h-5 bg-red-500 text-white text-[10px] rounded-full flex items-center justify-center font-bold">{cartCount}</span>}
            </Link>
          </div>
        </div>
      </div>

      {/* Categories - clean images, no circles, like profibuy.sk */}
          ...
          <div className="flex items-end overflow-x-auto no-scrollbar">
            {navCategories.map(({ cat, label, showMega }) => (
              <Link
                ...
                className={`flex flex-col items-center px-3 py-3 min-w-[105px] max-w-[125px] text-center group flex-shrink-0 border-b-2 transition-colors ${
              ...
              >
                <div className="w-14 h-14 flex items-center justify-center mb-1.5">
                  {cat.image ? (
                    ...
                    <span className="text-2xl text-gray-300">{label.charAt(0)}</span>
                  )}
                </div>
            ...
            {navCategories.length > 12 && (
              <Link href="/categories" className="flex flex-col items-center px-3 py-3 min-w-[60px] group flex-shrink-0 border-b-2 border-transparent">
                <div className="w-14 h-14 flex items-center justify-center mb-1.5">
                  <svg className="w-7 h-7 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 6h16M4 12h16M4 18h16" /></svg>
                </div>
                <span className="text-[11px] font-semibold text-gray-500">Ďalšie</span>
              </Link>
            )}
          </div>
        </div>
